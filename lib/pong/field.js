            ...
            for (var j = 0; j < n; ++j) {
                if (i !== j) {
                    ...
                    if (bboxIntersectsBBox(bboxI, bboxJ)) {
                        ...
                        if (bboxIntersectsBBox(calcBBox(colliders[i]), bboxJ)) {
                            var eNormal = circleNormal(bboxJ, oldPos);
                            while (bboxIntersectsBBox(calcBBox(colliders[i]), bboxJ)) {
                                colliders[i].state.x += eNormal.x * colliders[i].state.speed * delta;
                                colliders[i].state.y += eNormal.y * colliders[i].state.speed * delta;
                            }
                            if (colliders[j].actorType === "paddle")
                                colliders[j].state.value++;
                            colliders[i].state.vx = eNormal.x;
                            colliders[i].state.vy = eNormal.y;
                        } else {
