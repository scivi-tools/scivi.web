...
if (HAS_INPUT["Layer"] && INPUT["Layer"] &&
    IN_VISUALIZATION) {
    var getStyle = function (feature) {
        ...
        if (isNaN(val)) {
            ...
            var colorFrom = hex2color(SETTINGS_VAL["From Color"]);
            var colorTo = hex2color(SETTINGS_VAL["To Color"]);
            var minVal = INPUT["Min"];
            var maxVal = INPUT["Max"];
            var v = val;
            if (SETTINGS_VAL["Log Scale"]) {
                minVal = minVal > 0 ? Math.log(minVal) / Math.log(10) : 0;
                maxVal = maxVal > 0 ? Math.log(maxVal) / Math.log(10) : 0;
                v = v > 0 ? Math.log(v) / Math.log(10) : 0;
            }
            var t = (v - minVal) / (maxVal - minVal);
            for (var i = 0; i < 3; ++i)
                color[i] = Math.round(colorFrom[i] + t * (colorTo[i] - colorFrom[i]));
        }
        return {
            weight: hl ? 5 : 2,
            opacity: 1,
    ...
    if (!CACHE["Legend"]) {
        ...
        legend.onAdd = function (map) {
            var div = L.DomUtil.create("div", "scivi_map_info scivi_map_legend");
            div.innerHTML = "<div>" + INPUT["Max"] + "</div><div style='background: linear-gradient(to top, " + 
                            SETTINGS_VAL["From Color"] + ", " + SETTINGS_VAL["To Color"] +
                            "); width: 30px; height: 80px; border: 1px solid #666; border-radius: 5px;'></div><div>" +
                            INPUT["Min"] + "</div>";
            return div;
        };
        layer.push(legend);
