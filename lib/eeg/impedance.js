...
var getReferenceMap = function (svg) {
    var myMap = new Map();
    var keys = Object.keys(svg.all);
    for (var i = 0; i < keys.length; ++i) {
	myMap.set(svg.all[i].id.toLowerCase(), svg.all[i]);
    }
...
if (IN_VISUALIZATION) {
    ...
    if (!electrodes) {
        ...
        electrodes.style = "display: inline-block; max-height:800px;"; // HACK: limit height
    ...
    if (loaded && HAS_INPUT["EEG"]) {
        ...
        var elem_map = CACHE["References"];
	if (!elem_map) {
	    elem_map = getReferenceMap(electrodes.contentDocument);
	    CACHE["References"] = elem_map;
	}
	    ...
	    var el = elem_map.get(elName.toLowerCase());
            const offset = 5;
            if (el) {
                el.style = elVal < 0 ? 
                           "fill: #929292" : 
