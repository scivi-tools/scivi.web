...
function classifyData(testMap, patterns) {
    ...
    globalPossibilities = Object.keys(globalPossibilities).map(region => `${region}: ${globalPossibilities[region]}`);
    globalPossibilities.sort((l, r) => +r.split(": ")[1] - +l.split(": ")[1]);
...
if (IN_VISUALIZATION && HAS_INPUT["Pattern Maps"] && HAS_INPUT["Test Maps"]) {
    ...
    let result = {};
    test.forEach((t, i) => { result["Map " + (i + 1)] = classifyData(t, patterns); });
