...
function ping()
{
    var values = SETTINGS["SETTING_NAME"];
    var val = SETTINGS_VAL["SETTING_NAME"];
    if (values && val !== undefined && val < values.length) {
        var ws = new WebSocket("ws://" + values[val] + ":81");
        ws.onopen = function (evt) {
            ws.send(Uint8Array.from([ 0x0D ]));
            ws.close();
        }
    }
}
