...
function buildSubset(stats, subsetIndex, res, resID, legend)
{
    ...
    for (let i = 0, n = dims !== undefined ? dims.length : 0; i < n; ++i)
    {
        result += `
            <div class="report-slider-container">
                <div>${dims[i].name}:</div>
                <div class="report-subsets-slider" id="${resID}_${subsetIndex}_${i}_dimension"/></div>
            </div>
        `;
    }
    for (let i = 0, j = 0, k = 0, n = subset.slices.length; i < n; ++i, ++k)
        result += buildSubsetSlice(subset.slices[i], subsetIndex, i, res, resID, legend);
    return result;
}
...
function selectSlice(subsetIndex, dims, resID)
{
    ...
    for (let i = 0, n = dims.length; i < n; ++i)
    {
        ...
        const value = dims[i].ticks.indexOf(slider.noUiSlider.get());
