...
async function plotHeatmap(xAxisName, yAxisName, dataURL, minV, maxV, canvas)
{
    await ReportHelpers.createChart((chart) => {
        const scale = {
...
function cmosHeatmap(n, stats, coord, res)
{
    ...
    return `
        ...
        <div class="report-detector-dashrow">
            <div onclick="NChart3DLib.chart.${canvasID}();">
                <canvas id="${canvasID}" style="width: ${ReportHelpers.calcChartWidth(4, 0, 50)}px; height: 200px;"></canvas>
            </div>
        </div>
    `;
}
