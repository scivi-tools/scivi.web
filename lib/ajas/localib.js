...
function makeLODs(seriesType, color, lods)
{
    const chartLODs = [];
...
async function plot(caption, valuesLODs, fitLODs, residualsLODs, valuesColor, cvsIDBase, tsURL)
{
    ...
    await ReportHelpers.createChart((chart) => {
        ...
        chart[cvsIDBase] = () => {
            ...
            chartConfigValues.series[0] = makeLODs("band", valuesColor, valuesLODs);
            chartConfigValues.series[1] = makeLODs("line", "#B51700", fitLODs);

            chartConfigResiduals.cartesianSystem.margin = [ 10, 10, 10, 10 ];
            chartConfigResiduals.series[0] = makeLODs("band", "#004D80", residualsLODs);
        ...
        ReportHelpers.renderChart(chart, chartConfigValues, cvsIDBase + "_val");
        ReportHelpers.renderChart(chart, chartConfigResiduals, cvsIDBase + "_res");
    });
}
    ...
    for (let p = 0, pCnt = stats.params.length; p < pCnt; ++p)
    {
        for (let n = 0; n < 4; ++n)
        {
            await plot(`Calibration Parameters Δ𝜂${stats.params[p].name} of ${stats.params[p].detectors[n].name}`,
                       ...
                       canvasIDBase(stats.params[p].name, stats.params[p].detectors[n].name, "eta"),
                       stats.timestamps);
            await plot(`Calibration Parameters Δ𝜁${stats.params[p].name} of ${stats.params[p].detectors[n].name}`,
                       ...
                       canvasIDBase(stats.params[p].name, stats.params[p].detectors[n].name, "zeta"),
                       stats.timestamps);
        }
    }
}
