...
async function plotMap(caption, dataPath, minVal, maxVal, minX, maxX, minY, maxY, xAxisName, yAxisName, interestRegion, canvas)
{
    await ReportHelpers.createChart((chart) => {
        const scale = {
        ...
        chart[canvas] = () => {
            ...
            SCIVI.forkWithTemplate(ReportHelpers.scatterTemplate(chartConfig));
        };
        ReportHelpers.renderChart(chart, chartConfig, canvas);
    });
}
...
async function buildSources(solutionID)
{
    ...
    const html = `
            ...
            <div class="report-dashbox">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td class="table-num">${stats["upsilonNonGaiaGaussSigma"].toExponential(3)}</td>
                    </tr>
                    <tr>
                        <td>𝜌</td>
                        ...
                        <td class="table-num">${stats["rhoNonGaiaGaussSigma"].toExponential(3)}</td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="report-dashrow" style="margin-top: 20px;">
            <div class="report-dashbox">
                ...
                <div style="width: ${ReportHelpers.calcChartWidth(2, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.upsilonUpdatesHist();">
                    <canvas id="upsilonUpdatesHist" style="width: 100%; height: 100%;"></canvas>
                </div>
            </div>
            <div class="report-dashbox">
                <h3>Updates Histogram for 𝜌</h3>
                <div style="width: ${ReportHelpers.calcChartWidth(2, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.rhoUpdatesHist();">
        ...
        <div class="report-dashrow" style="margin-top: 20px;">
            <div class="report-dashbox">
                ...
                <div style="width: ${ReportHelpers.calcChartWidth(4, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.upsilonUpdatesGRSMap();">
                    <canvas id="upsilonUpdatesGRSMap" style="width: 100%; height: 100%;"></canvas>
                </div>
            </div>
            <div class="report-dashbox">
                <h3>Updates Map for 𝜌 [μas] in GRS</h3>
                <div style="width: ${ReportHelpers.calcChartWidth(4, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.rhoUpdatesGRSMap();">
                    <canvas id="rhoUpdatesGRSMap" style="width: 100%; height: 100%;"></canvas>
                </div>
            </div>
            <div class="report-dashbox">
                <h3>Updates Map for 𝜐 [μas] in JORS</h3>
                <div style="width: ${ReportHelpers.calcChartWidth(4, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.upsilonUpdatesJORSMap();">
                    <canvas id="upsilonUpdatesJORSMap" style="width: 100%; height: 100%;"></canvas>
                </div>
            </div>
            <div class="report-dashbox">
                <h3>Updates Map for 𝜌 [μas] in JORS</h3>
                <div style="width: ${ReportHelpers.calcChartWidth(4, 0, 0)}px; height: 400px;" onclick="NChart3DLib.chart.rhoUpdatesJORSMap();">
    ...
    await plotMap("Updates Map for 𝜐 [μas] in GRS",
                  stats["pathUpsilonGRS"], stats["minUpsilonUpdate"], stats["maxUpsilonUpdate"],
    ...
    await plotMap("Updates Map for 𝜌 [μas] in GRS",
                  stats["pathRhoGRS"], stats["minRhoUpdate"], stats["maxRhoUpdate"],
                  stats["minL"], stats["maxL"], stats["minB"], stats["maxB"],
                  "Galactic Longitude [deg]", "Galactic Latitude [deg]",
                  interestRegionGRS,
                  "rhoUpdatesGRSMap");
    await plotMap("Updates Map for 𝜐 [μas] in JORS",
                  stats["pathUpsilonJORS"], stats["minUpsilonUpdate"], stats["maxUpsilonUpdate"],
    ...
    await plotMap("Updates Map for 𝜌 [μas] in JORS",
                  stats["pathRhoJORS"], stats["minRhoUpdate"], stats["maxRhoUpdate"],
                  stats["minUpsilon"], stats["maxUpsilon"], stats["minRho"], stats["maxRho"],
                  "JORS 𝜐 [deg]", "JORS 𝜌 [deg]",
                  interestRegionJORS,
                  "rhoUpdatesJORSMap");
