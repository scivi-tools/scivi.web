...
var hasInput = HAS_INPUT["Corpus"] && INPUT["Corpus"];
var clearStartText = false;

if (hasInput) {
    if (SETTINGS_VAL["Start Text"] === undefined) {
        var texts = [];
        var corpus = INPUT["Corpus"]["corpus"];
        if (corpus) {
            for (var i = 0, n = corpus.length; i < n; ++i)
                texts.push(corpus[i]["name"]);
            SETTINGS["Start Text"] = texts;
            SETTINGS_VAL["Start Text"] = 0;
        } else {
            clearStartText = true;
        }
    }
...
if (clearStartText) {
    SETTINGS["Start Text"] = undefined;
    SETTINGS_VAL["Start Text"] = undefined;
}
...
if (IN_VISUALIZATION) 
{
        ...
        (!CACHE["texts"] || !corpusesAreEqual(CACHE["texts"], INPUT["Corpus"]["corpus"])))
    {
        ...
        CACHE["current_text_index"] = SETTINGS_VAL["Start Text"];
        drawText(CACHE["current_text_index"]);
    }

    if (!CACHE["toolbar"] && SETTINGS_VAL["Show Buttons"])
    {
