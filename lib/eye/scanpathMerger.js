...
function buildMergedScanpath(fixations, size, aois)
{
    ...
    for (var i = 0, n = fixations.length; i < n; ++i) {
        ...
        for (var j = 0, m = mf.length; j < m; ++j) {
            ...
            for (var k = 0, l = aois.length; k < l; ++k) {
                if (hitTest(pt, aois[k])) {
                    if (path.length === 0 || path[path.length - 1].aoiID !== k)
                        path.push({ aoiID: k, time: mf[j][2] });
                    else
                        path[path.length - 1].time += mf[j][2];
                    break;
                }
            }
        }
    ...
    for (var i = 0; i < maxScanpathLength; ++i) {
        var pathStep = [ [ "AOI ID", "Value", "Average Fixation Time" ] ];
        for (var j = 0, n = paths.length; j < n; ++j) {
            if (paths[j].length > i) {
                var idx = findAOI(pathStep, paths[j][i].aoiID);
                if (idx >= 0) {
                    pathStep[idx][1]++;
                    pathStep[idx][2] += paths[j][i].time;
                } else {
                    pathStep.push([ paths[j][i].aoiID, 1, paths[j][i].time ]);
                }
            }
        }
...
if (IN_VISUALIZATION) {
    ...
    // OUTPUT["AOIs Order"] = aoisOrder; // FIXME: choose by slider from mergedScanpath
} else {
