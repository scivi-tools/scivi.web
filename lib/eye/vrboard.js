...
if (IN_VISUALIZATION) 
{
    ...
    if (CACHE["Gaze"])
    {
        ...
        OUTPUT["Gaze"] = {"picture_name": CACHE["CurrentImage"].title, "data": CACHE["Gaze"]};
        CACHE["Gaze"] = null;
    }
    if (CACHE["WAV"])
    {
    ...
    if (CACHE["WallLog"])
    {
        OUTPUT["Wall Log"] = {"title": CACHE["CurrentImage"].title, 
                                "row": CACHE["WallLog"]};
        CACHE["WallLog"] = null;
    }
        ...
        (!CACHE["CurrentImage"] || CACHE["CurrentImage"].image !== INPUT["Picture"])) 
    {
        CACHE["newImageToSend"] = { title: INPUT["Title"],
                                        ...
                                        AOIs: INPUT["AOIs"] };
        CACHE["RestTimeBeforeSendImage"] = DOT_TIMEOUT;//set timer
        let ws = CACHE["webSocket"]
        if (ws) SetNullPhase();
        else CACHE["SetNULLPhase"] = true;
    }
