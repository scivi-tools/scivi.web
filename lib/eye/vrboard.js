...
function addCmdButton(name, command)
{
    ...
    btn.addEventListener("click", (evt) => {
        if (CACHE["webSocket"])
            CACHE["webSocket"].send(JSON.stringify(command(btn)));
    });
    return btn;
}
...
if (IN_VISUALIZATION) 
{
    ...
    if (!CACHE["container"]) 
    {
        ...
        toolbar.appendChild(addCmdButton("Calibrate",  (btn) => {return { calibrate: true };} ));
        toolbar.appendChild(addCmdButton("Custom Caibrate", (btn) => {return { customCalibrate: true };} ));
