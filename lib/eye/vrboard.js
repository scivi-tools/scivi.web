...
if (IN_VISUALIZATION) {
    ...
    if (!CACHE["container"]) {
        ...
        var customCalib = document.createElement("div");
        customCalib.innerHTML = "Custom Caibrate";
        customCalib.classList.add("scivi_button");
        customCalib.addEventListener("click", function (e) {
            var vrSrvIP = SETTINGS_VAL["VR Server IP"];
            if (vrSrvIP.indexOf(":") === -1)
                vrSrvIP += ":81";
            var ws = new WebSocket("ws://" + vrSrvIP + "/ue4");
            ws.onopen = function (evt) {
                ws.send(JSON.stringify({ customCalibrate: true }));
            }
        });
        toolbar.appendChild(customCalib);
