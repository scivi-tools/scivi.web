...
function assembleScale(walls)
{
    ...
    for (var i = 1, n = lastMeaningful(walls); i <= n; ++i) {
        if (walls[i][1] === "NewWall") {
            ...
            var scaleItemID = scaleItemIDByName(scale, walls[i][2]);
            var aoiID = parseInt(walls[i][3]) + 1;
...
if (IN_VISUALIZATION && HAS_INPUT["Gaze"] && HAS_INPUT["Walls"] && HAS_INPUT["AOIs"]) {
    ...
    var keywords = [];
    for (var i = 2, n = lastMeaningful(walls); i <= n; ++i) {
        if (walls[i][1] === "DeleteWall") {
            keywordsPhaseEnd = parseInt(walls[i][0]);
            break;
        } else if (walls[i][1] === "AddAOI") {
            keywords.push(parseInt(walls[i][3]) + 1);
        }
    }
    ...
    var index = assembleEdges(1, readingPhaseEnd, nodesReading, edgesReading, gaze);
    ...
    OUTPUT["Scale"] = assembleScale(walls);
}
