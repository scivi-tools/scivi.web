...
if (IN_VISUALIZATION) {
    ...
    if (HAS_INPUT["Table"] && INPUT["Table"] && CACHE["rec"]){
        let file_name = INPUT["Table"]["title"];
        let row = INPUT["Table"]["row"];
        if (!CACHE["csv_files"][file_name])//if no that file, then insert
            CACHE["csv_files"][file_name] = [INPUT["Table"]["header"] || []];
        CACHE["csv_files"][file_name].push(row);
        
    }
} else {
    CACHE["rec"] = false;
    CACHE["csv_files"] = null;
}
