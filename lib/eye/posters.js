...
if (IN_VISUALIZATION) {
    if (HAS_INPUT["Gaze"] && (INPUT["Gaze"] !== null) && (DATA["Images"].length === DATA["Image Count"])) {
        var gaze = INPUT["Gaze"];
        if (gaze[IDX_TYP] === "R_RELD") {
            ...
            else if (gaze[IDX_TIM] - CACHE["DotEnterTS"] > DOT_TIMEOUT && CACHE["PosterIndex"] < DATA["Image Count"]) {
