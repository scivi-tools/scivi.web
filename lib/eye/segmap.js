...
function getRootUuid(figures, uuid) {
    if (figures[uuid]["parentUuid"]) {
        return getRootUuid(figures, figures[uuid]["parentUuid"]);
    }
    return uuid;
}

function getChildrenUuidsContainParameter(figures, uuid, parameter) {
    ...
    if (figure["tool"] !== "group") {
        ...
        let result = [];
        if (figure[parameter] !== undefined) {
            result.push(uuid);
        }

        figure["children"].map(u => {
            let children = [];
            let dirtyChildren = getChildrenUuidsContainParameter(figures, u, parameter);
            if (dirtyChildren?.map) {
                dirtyChildren.map(child => child && children.push(child));
            } else if (typeof(dirtyChildren) === "string") {
                children.push(dirtyChildren);
            }
...
function buildContoursTreeByUuidTree(figures, uuidTree) {
    if (typeof(uuidTree) === "object" && !(uuidTree instanceof Array)) { //root level
        return {contours: Object.keys(uuidTree).map(uuid => buildContoursTreeByUuidTree(figures, uuidTree[uuid]))};
    } else if (typeof(uuidTree) === "object") { //in-between level
        return {children: uuidTree.map(uuid => buildContoursTreeByUuidTree(figures, uuid))};
    } else { // leaf
        ...
        let approximatedPath = approximateLinearly(transformPath(figures[uuidTree].points, figures[uuidTree].x, figures[uuidTree].y, 1, 1));
        ...
        return {name: figures[uuidTree].name, path: approximatedPathWithGroupedPoints};
    }
}
...
function convertStudioMapToSciViFormat(jsonMap)
{
    ...
    Object.keys(figures).map((uuid, i) => {
        let figure = figures[uuid];
        if (figure.points) {
            let root = getRootUuid(figures, uuid);
            if (!uuidTree[root]) {
                uuidTree[root] = getChildrenUuidsContainParameter(figures, root, "points");
            }
        }
        return undefined;
    })
    let contoursTree = buildContoursTreeByUuidTree(figures, uuidTree);
    contoursTree.image = jsonMap.menu.backgroundImage || "";
    console.log(contoursTree);
    return contoursTree;
}
...
function encloses(cA, cB, epsilon)
{
    if (isParent(cB, cA) || fakeArea(cA) < fakeArea(cB) || !bboxIntersection(cA.bbox, cB.bbox))
        return false;
    if (!cB.erodedPath)
        cB.erodedPath = erode(cB.path, epsilon);
