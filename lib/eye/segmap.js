...
//             (pt[0] < (poly[j][0] - poly[i][0]) * (pt[1] - poly[i][0]) / (poly[j][1] - poly[i][1]) + poly[i][0]))
//             c = !c;
//     }
//     return c;
// }
...
/**
 * Centroid is formatted as [ x, y ].
 */
function centroid(poly)
{
    var result = null;
    if (poly.length > 0) {
        result = [ poly[0][0], poly[0][1] ];
        for (var i = 1; i < poly.length; ++i) {
            result[0] += poly[i][0];
            result[1] += poly[i][1];
        }
        result[0] /= poly.length;
        result[1] /= poly.length;
    }
    return result;
}
...
function dilate(poly, epsilon)
{
    return poly.path;
    var result = [];
    for (var i = 0; i < poly.path.length; ++i) {
        var vx = poly.path[i][0] - poly.centroid[0];
        var vy = poly.path[i][1] - poly.centroid[1];
        var vl = Math.sqrt(vx * vx + vy * vy);
        vx /= vl;
        vy /= vl;
        result.push([ poly.path[i][0] + vx * epsilon, poly.path[i][1] + vy * epsilon ]);
    }
    return result;
}
...
function encloses(cA, cB, epsilon)
{
    if (isParent(cB, cA) || fakeArea(cA) < fakeArea(cB) || !bboxIntersection(cA.bbox, cB.bbox))
        return false;
    if (!cA.centroid)
        cA.centroid = centroid(cA.path);
    if (!cA.dilatedPath)
        cA.dilatedPath = dilate(cA, epsilon);
...
function makeHierarchy(contours)
{
    ...
    var epsilon = 0.1 * Math.min(totalBBox[2] - totalBBox[0], totalBBox[3] - totalBBox[1]); // 0.1 here can be a setting
    for (var i = 0; i < contours.length; ++i) {
