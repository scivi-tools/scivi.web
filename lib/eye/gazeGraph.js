...
if (IN_VISUALIZATION) {
    ...
    if (HAS_INPUT["AOIs"] && SCIVI.nonNull(INPUT["AOIs"])) {
        var aois = INPUT["AOIs"];
        var nodeID = 1;
    ...
    if (HAS_INPUT["Gaze Data"] && SCIVI.nonNull(INPUT["Gaze Data"])) {
        var gaze = INPUT["Gaze Data"];
        var prevAOI = -1;
        var precAOI = -1;
        var lastTime = 0;
        for (var i = 1, n = gaze.length; i < n; ++i) {
            if (gaze[i].length > 0) {
        ...
        var totalTime = lastTime - parseInt(gaze[1][IDX_TIM]);
