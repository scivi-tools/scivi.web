...
function smoothstep(min, max, procent)
{
    return min + (max - min) * procent;
}

function filterAreas(aois, minFiltered, maxFiltered)
{
    console.log('filter', minFiltered, maxFiltered);
    //set colors in aois
    const values = CACHE["Values"];
    const [minVal, maxVal] = CACHE["ValuesRange"];
    minFilteredVal = smoothstep(minVal, maxVal, minFiltered * 0.01)
    maxFilteredVal = smoothstep(minVal, maxVal, maxFiltered * 0.01)
