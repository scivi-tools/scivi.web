...
function paintAreas(aois, values, minLbl, maxLbl)
{
    ...
    circles = [];
    var total = 0.0;
    // find max and min values and times
    for (var i = 1, n = values.length; i < n; ++i) {
    ...
    CACHE["ValuesRange"] = [minVal, maxVal];
    $("#colorScaleSlider").slider("option", "values", [0, 100]);
    // draw some circles
    for (var i = 1, n = values.length; i < n; ++i) 
    {
...
function filterAreas(aois, minFiltered, maxFiltered)
{
    //set colors in aois
    const values = CACHE["Values"];
    const [minVal, maxVal] = CACHE["ValuesRange"];
    minFilteredVal = lerp(minVal, maxVal, minFiltered * 0.01)
    maxFilteredVal = lerp(minVal, maxVal, maxFiltered * 0.01)
    const handle_value0 = document.getElementById('handle_0_value');
    const handle_value1 = document.getElementById('handle_1_value');
    if (handle_value0)
        handle_value0.innerText = minFilteredVal.toFixed(2);
    if (handle_value1)
        handle_value1.innerText = maxFilteredVal.toFixed(2);
    const eps = 0.00001;
    for (var i = 0, n = aois.length; i < n; ++i) 
    {
        var idx = findValuesIndex(values, i);
        if (idx === -1 || values[idx][IDX_AOI_VALUE] > maxFilteredVal + eps
                         || values[idx][IDX_AOI_VALUE] < minFilteredVal - eps)
            aois[i].svgPath.setAttribute("fill", "url(#diagonalHatch)");
        else 
            aois[i].svgPath.setAttribute("fill", fillColor(values[idx][IDX_AOI_VALUE], minFilteredVal, maxFilteredVal));
    }
}

function showValuesNearHandles(slider)
{
    const handles = $("#colorScaleSlider .ui-slider-handle")
    for(var i = 0, c = handles.length; i< c; ++i)
    {
        var span = document.createElement('span');
        span.id = `handle_${i}_value`;
        span.style.color = 'black';
        span.style.marginLeft = '150%';
        span.style.textAlign = 'center';
        handles[i].appendChild(span);
    }
}

function hideValuesNearHandles(slider)
{
    const  handles = $("#colorScaleSlider .ui-slider-handle")
    for(var i = 0, c = handles.length; i< c; ++i)
        handles[i].innerHTML = '';
...
function createContainer(pic, aois, values)
{
	...
	var downloadTableButton = document.createElement('button');
	downloadTableButton.innerText = "Download table";
	downloadTableButton.style.display = 'block';
	downloadTableButton.style.marginLeft = '25rem';
	downloadTableButton.classList.add("scivi_button");
	downloadTableButton.addEventListener("click", () => {
		console.log(aois, values);
		const data = values[n-1];
		const rows = [data[0]];
		for(let i = 1; i < data.length; ++i)
		{
			console.assert(data[i].length == 2);
			console.assert(aois[data[i][0]]);
			const aoi_name = aois[data[i][0]].name;
			rows.push([aoi_name, data[i][1]]);
		}
		SAVE_FILE(dumpCSV(rows), "result.csv", ".csv", "text/plain;charset=utf-8", true);
	});
	sliderContainer.appendChild(downloadTableButton);
    ...
    var colorScaleSlider = document.createElement('div');
    colorScaleSlider.id = "colorScaleSlider";

    $(colorScaleSlider).slider({
        animate: "slow",
        range: true,    
        values: [0, 100],
        min: 0,
        ...
        slide: (event, ui) => filterAreas(aois, ui.values[0], ui.values[1]),
        start: (event, ui) => showValuesNearHandles(colorScaleSlider),
        stop: (event, ui) => hideValuesNearHandles(colorScaleSlider)
    }).css({
        "background": "gray",
        "width": "50px",
        "height": "250px"
    ...
    $(".ui-slider-range", colorScaleSlider).css({
        "background-image": "linear-gradient(hsl(0, 100%, 50%), \
                            hsl(32, 100%, 50%), hsl(64, 100%, 50%), \
                            hsl(128, 100%, 50%), hsl(180, 100%, 50%))"
    });

    $(".ui-slider-handle", colorScaleSlider).css({
        "margin-left": "100%",
        "margin-bottom": "-2px",
        "height": "4px"
    });
    ...
    colorScaleLegendContainer.append(colorScaleSlider);
