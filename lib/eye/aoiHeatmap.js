...
function colorScale(cFrom, cTo, value, minVal, maxVal)
{
    var t = minVal === maxVal ? 0.0 : (value - minVal) / (maxVal - minVal);
    for (var i = 0; i < 3; ++i)
        cFrom[i] += t * (cTo[i] - cFrom[i]);
    return cFrom;
}

function circleFillColor(value, minVal, maxVal)
{
    var c = colorScale([ 255, 255, 0 ], [ 255, 0, 0 ], value, minVal, maxVal);
    return "rgba(" + c[0] + "," + c[1] + "," + c[2] + ",0.5)";
}

function circleStrokeColor(value, minVal, maxVal)
{
    var c = colorScale([ 255, 255, 0 ], [ 255, 0, 0 ], value, minVal, maxVal);
...
{
    console.log(value + " " + minVal + " " + maxVal);
    var rFrom = 5;
    var rTo = 30;
    var t = minVal === maxVal ? 0.0 : (value - minVal) / (maxVal - minVal);
    console.log(t);
...
function paintAreas(aois, values, minLbl, maxLbl)
{
    ...
    circles = [];
    for (var i = 2, n = values.length; i < n; ++i) {
    ...
    for (var i = 1, n = values.length; i < n; ++i) {
        var c = getNode("circle", { cx: values[i][IDX_AOI_AVU] * size[0],
                                    ...
                                    fill: circleFillColor(values[i][IDX_AOI_VALUE], minVal, maxVal),
                                    stroke: circleStrokeColor(values[i][IDX_AOI_VALUE], minVal, maxVal),
