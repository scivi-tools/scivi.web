...
function storeGaze(corrU, corrV, rawU, rawV, errCorr, errRaw)
{
    var u = CACHE["realU"];
    var v = CACHE["realV"];
    CACHE["data"].push([u, v, corrU, corrV, rawU, rawV, errCorr, errRaw]);
}
...
if (IN_VISUALIZATION) {
    ...
    if (HAS_INPUT["Gaze"] && INPUT["Gaze"] !== null /*&& !SETTINGS_VAL["Free Mode"] && CACHE["oldFullGrid"]*/) {
        ...
        if (gaze[IDX_TYP] === "R_RELD") {
            storeGaze(gaze[IDX_U], gaze[IDX_V], gaze[IDX_RU], gaze[IDX_RV], gaze[IDX_CALPHA], gaze[IDX_CBETA]);
            updateView(true);
        }
    }
} else {
